...
const ShoppingADAutoBidContainer = () => {
    ...
    useEffect(() => setCustomer(JSON.parse(localStorage.getItem("customer"))), []);
    ...
    const onAddAutoBid = async () => {
        ...
        try {
            const res = await SendRequest().post(`${serverPROTOCOL}${serverURL}/autobid/shopping_ad?CUSTOMER_ID=${customer["CUSTOMER_ID"]}`, keywordOption);
            console.info(res);
        } catch(e) {
            throw new Error(e);
        }
